...
struct LCPath {
    ...
    public static let lcGroupDocPath = {
        ...
        guard let appGroupPathUrl = LCSharedUtils.appGroupPath() else {
            // VibeContainers deliberately runs without a SideStore/AltStore
            // app-group entitlement. Its canonical storage is Documents, so
            // probing a hard-coded group only produces containerd violations.
            return docPath
        }
