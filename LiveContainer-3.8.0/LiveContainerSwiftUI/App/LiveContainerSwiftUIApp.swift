...
struct LiveContainerSwiftUIApp : SwiftUI.App {
    ...
    var body: some Scene {
        ...
        if UIApplication.shared.supportsMultipleScenes, #available(iOS 16.1, *) {
            WindowGroup(id: MultitaskGuestScene.windowGroupID, for: String.self) { $id in
                if let id {
                    MultitaskGuestScene(id: id, fallbackHostWindowGroupID: "Main")
                }
            }
    ...
    private var mainWindowContent: some View {
        ...
        if #available(iOS 16.1, *), UIApplication.shared.supportsMultipleScenes {
            content.modifier(MultitaskWindowRequestModifier())
        } else {
            content
        }
    }
