...
- (nullable NSURL *)hook_containerURLForSecurityApplicationGroupIdentifier:(NSString *)groupIdentifier {
    ...
    } else if (NSUserDefaults.isLiveProcess && getenv("LC_LIVEPROCESS_APP_GROUPS_PATH")) {
        // The host grants this canonical directory to LiveProcess alongside
        // the selected data container. Using it keeps normal and multitask
        // launches on the same app-group Core Data store while avoiding the
        // ungranted Documents alias that produced Cocoa error 134080.
        result = [NSURL fileURLWithPath:[NSString stringWithFormat:@"%s/%@",
            getenv("LC_LIVEPROCESS_APP_GROUPS_PATH"), groupIdentifier]];
