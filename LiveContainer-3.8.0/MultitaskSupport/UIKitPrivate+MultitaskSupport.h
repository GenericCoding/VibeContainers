...
@class RBSProcessIdentity, FBProcessExecutableSlice, UIMutableApplicationSceneClientSettings, UIMutableScenePresentationContext, UIScenePresentationManager, _UIScenePresenter, _UISceneEventDeferringHostComponent, _UISceneRelationshipManagementHostComponent, FBSceneSnapshot, _UISceneSnapshotPresentationView;
...
- (FBSceneSnapshot *)createSnapshot;
@end

/// FrontBoard snapshots render the guest's cross-process CAContext. UIKit view
/// hierarchy drawing only sees the local host layers and therefore produces a
/// blank card for a hosted LiveProcess scene.
...
@property(nonatomic, strong) FBSceneSnapshot *sceneSnapshot;
- (instancetype)initWithSnapshot:(FBSceneSnapshot *)snapshot;
- (BOOL)capture;
@end
...
- (_UISceneSnapshotPresentationView *)captureSnapshotPresentationView;
- (void)activate;
- (void)deactivate;
- (void)invalidate;
@end
